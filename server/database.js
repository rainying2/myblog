...
// 创建数据库
let createDb=`create database if not exists myblog character set utf8 collate utf8_general_ci`;

//使用数据库
let useDb=`use myblog`;
...
//插入文章
let insertArticle=`
    insert into article (title,time,category,tags,state,summary,content) values('这是技术文章测试','2020-6-21','技术文章','html','发表','这是一个简介','这是一个内容');
    `;
...
let insertComments=`insert into comments (articleId,name,content) values (1,'不二D荧林仕','跪求大佬带带我');`;
...
let insertUser=`insert into user (account,password,name,signature,breif) values ('admin','812bf295db843ad39280f48332f3a416','不二','Against the current','永远都不要放弃挣扎');`;
